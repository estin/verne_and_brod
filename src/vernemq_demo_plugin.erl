...
auth_on_publish(UserName, {_MountPoint, _ClientId} = SubscriberId, QoS, Topic, Payload, IsRetain) ->
    ...
    %% we return 'ok'
    KafkaTopic = <<"fd">>,
    MessageId = crypto:hash(md5, Payload),
    PartitionFun = fun(_Topic, PartitionsCount, _Key, _Value) -> {ok, crypto:rand_uniform(0, PartitionsCount)} end,
    AckCb = fun(CbPartition, CbBaseOffset) -> error_logger:info_msg("[brod] Produced ~p to ~p partition ~p at base-offset ~p", [MessageId, KafkaTopic, CbPartition, CbBaseOffset]) end,
    Result = brod:produce_cb(kafka_client, KafkaTopic, PartitionFun, MessageId, Payload, AckCb),
    error_logger:info_msg("[brod] produce_cb result ~p", [Result]),
    ok.
